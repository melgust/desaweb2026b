...
public class ClientsController : ControllerBase
{
    ...
    public async Task<ActionResult<ClientPagedResult>> GetClients([FromQuery] string? search, [FromQuery] string? sortBy, [FromQuery] string? sortDirection, [FromQuery] int page = 1, [FromQuery] int pageSize = 10, CancellationToken ct = default)
        => Ok(await _clientService.GetClientsAsync(search, sortBy, sortDirection, page, pageSize, ct));

    [HttpGet("{id:guid}")]
    [Authorize(Roles = "Admin,Manager,User")]
    public async Task<ActionResult<ClientDto>> GetById(Guid id, CancellationToken ct)
        => Ok(await _clientService.GetByIdAsync(id, ct));
    ...
    public async Task<ActionResult<ClientDto>> Update(Guid id, [FromBody] UpdateClientRequest request, CancellationToken ct)
        => Ok(await _clientService.UpdateAsync(id, request, ct));
